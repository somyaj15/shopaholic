...
{name:"GLORIOUS LONG DRESS" , image:"https://i.pinimg.com/236x/f2/d3/f7/f2d3f7334e04d034422f98193f66134c.jpg" ,
 description: "Style :- This suit is Ready to wear. Just select your size & order.    Fabric:-Faux Georgette .     Care Instructions:- Wash with hands or dry clean!!.   Size available-S,M,L and XL. " , price:"25" },
...
app.post("/shop",isAdmin,function(req,res){
    var name=req.body.name
    var image=req.body.image
    var price=req.body.price
    var description=req.body.description
    var newShop={name:name , image:image , price:price,description:description}
...
//------------------------------------------------------------------------------------
    app.post("/cart",isLoggedIn,function(req,res){
        ...
        var buyer=req.id
        var newCart={name:name , image:image , price:price, buyer:buyer}
        ...
        Cart.create(newCart,function(err,newCreated){
         ...
         /* function del(){
            Cart.remove({buyer:buyer},function(err,res){
                  if(err)
                  {
                      console.log(err);
                  } else {
